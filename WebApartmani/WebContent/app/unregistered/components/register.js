Vue.component("register", {
    ...
    methods: {
        submit(event) {
            ...
            if (this.username.state && this.password.state && this.gender.state && this.name.state && this.surname.state) {
                axios.post("/WebApartmani/rest/auth/register",
                    {
                        ...
                        role: "Gost"
                    }).then(response => this.registered()).catch(
                        error => { this.alert.text = "Registracija nije uspela: " + error.response.data; this.alert.show = true; }
                    );
            }
        },
        registered(){
        	axios.post("/WebApartmani/rest/auth/login",
                    {   username: this.username.value,
                        password: this.password.value
                    }).then(response => window.location.href = "http://localhost:8081/WebApartmani/guest.html").catch(
  ...
  <b-form v-on:submit="submit" v-on:reset="reset">
    ...
    <b-form-group label="Pol">
      ...
      <b-button class="mt-2 mb-2 float-right" type="submit" variant="primary" :disabled="!formValid">Registruj se</b-button>
